...
// idcustomer viene como param en la URL: /dashboard/planes/:idcustomer
const idcustomer = Number(route.params.idcustomer)
// El nombre del cliente viaja en history.state (pasado desde DashboardView)
...
const obtenerPlanes = async () => {
  ...
  try {
    ...
    // GET /api/plans?idcustomer=X — filtra en backend por idcustomer
...
const guardarPlan = async () => {
  ...
  try {
    ...
    resetForm()
    await obtenerPlanes()   // recargar tabla
...
const estadoClass = (state) => {
  ...
  if (state.toLowerCase() === 'pagado') return 'badge-activo'
  if (state.toLowerCase() === 'pendiente') return 'badge-inactivo'
    ...
    <div class="layout">
      ...
      <aside class="sidebar">
        ...
        <div class="sidebar-footer">
          ...
          <button class="logout-btn" @click="() => { localStorage.removeItem('token'); router.push('/login') }" title="Cerrar sesión">
      ...
      <main class="main">
        ...
        <header class="page-header">
          <div class="page-title">
            ...
            <h1>{{ nombreCliente }} - {{ idcustomer }}</h1>
          </div>
          <div class="header-meta">
            ...
            <div class="stat-pill accent">
              <span class="stat-val">{{ planes.filter(p => p.state?.toLowerCase() === 'activo').length }}</span>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="(p, i) in planes" :key="p.idplan"
                  ...
                  <td>
                    <span class="id-badge">{{ p.idplan }}</span>
                  </td>
                  <td class="td-tipo">
...
.screen { min-height: 100vh; background: #0a0a0a; font-family: 'Barlow', sans-serif; position: relative; overflow: hidden; }
...
/* ── Main ── */
.main { flex: 1; padding: 40px 40px 60px; display: flex; flex-direction: column; gap: 24px; min-width: 0; }
...
.breadcrumb {
  ...
  color: #555; cursor: pointer; transition: color .2s;
  width: fit-content;
}
...
.page-title h1 { font-family: 'Barlow Condensed',sans-serif; font-size: 2.6rem; font-weight: 900; text-transform: uppercase; letter-spacing: .04em; color: #fff; line-height: 1; }
...
th { font-family: 'Barlow Condensed',sans-serif; font-size: .65rem; font-weight: 700; letter-spacing: .25em; text-transform: uppercase; color: #f5c500; padding: 14px 18px; text-align: left; white-space: nowrap; }

/* Celda especial con el botón Agregar Plan */
.th-add { text-align: right; padding-right: 14px; }

.btn-agregar {
  display: inline-flex; align-items: center; gap: 7px;
  background: #f5c500; border: none;
  color: #0a0a0a;
...
td { padding: 14px 18px; border-bottom: 1px solid rgba(255,255,255,.04); color: #ccc; vertical-align: middle; white-space: nowrap; }
...
.td-tipo .tipo-label { font-family: 'Barlow Condensed',sans-serif; font-weight: 700; font-size: .9rem; color: #f0f0f0; letter-spacing: .04em; }

.td-precio .precio-val { font-family: 'Barlow Condensed',sans-serif; font-weight: 700; font-size: .95rem; color: #f5c500; }
...
.badge-activo   { color: #4ade80; border-color: rgba(74,222,128,.4); background: rgba(74,222,128,.07); }
.badge-inactivo { color: #888; border-color: rgba(136,136,136,.3); background: rgba(136,136,136,.06); }
...
.empty-row { text-align: center; color: #444; padding: 56px 20px !important; font-size: .85rem; letter-spacing: .05em; }
...
.modal {
  ...
  box-shadow: 0 40px 100px rgba(0,0,0,.9), 0 0 60px rgba(245,197,0,.05);
...
.mfield input:focus,
.mfield select:focus { border-color: rgba(245,197,0,.5); background: #0f0e09; }
...
.btn-save {
  ...
  font-size: .8rem; font-weight: 900; letter-spacing: .2em; text-transform: uppercase;
  padding: 11px 28px; cursor: pointer; transition: all .2s;
}
