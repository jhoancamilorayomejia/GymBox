...
const usuariosFiltrados = computed(() =>
  users.value.filter(u => {
    ...
    const ok = u.username.toLowerCase().includes(q) ||
               u.rol.toLowerCase().includes(q) ||
               String(u.id).includes(q)
    return ok && (filtroRol.value === 'todos' || u.rol === filtroRol.value)
  })
)
...
/* ── Main: scroll interno, ocupa todo lo restante ── */
.main {
  flex: 1;
  height: 100vh;
  overflow-y: auto;
...
td { padding: 15px 20px; border-bottom: 1px solid rgba(255,255,255,.04); color: #ccc; vertical-align: middle; }
...
.avatar { width: 34px; height: 34px; background: linear-gradient(135deg,rgba(245,197,0,.2),rgba(255,122,0,.15)); border: 1px solid rgba(245,197,0,.2); color: #f5c500; font-family: 'Barlow Condensed',sans-serif; font-weight: 900; font-size: .95rem; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.pass-dots { color: #2e2e2e; letter-spacing: .15em; font-size: .85rem; }
...
.btn-plan { display: inline-flex; align-items: center; gap: 6px; background: transparent; border: 1px solid rgba(245,197,0,.3); color: #f5c500; font-family: 'Barlow Condensed',sans-serif; font-size: .72rem; font-weight: 700; letter-spacing: .18em; text-transform: uppercase; padding: 7px 16px; cursor: pointer; transition: all .2s; white-space: nowrap; }
